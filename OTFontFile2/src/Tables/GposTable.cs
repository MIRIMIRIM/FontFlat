...
[OtTable("GPOS", 10, GenerateTryCreate = false, GenerateStorage = false)]
public readonly partial struct GposTable
{
    private readonly TableSlice _table;
    private readonly OtlLayoutTable _layout;

    private GposTable(TableSlice table, OtlLayoutTable layout)
    {
        _table = table;
        _layout = layout;
    }

    public static bool TryCreate(TableSlice table, out GposTable gpos)
    {
        ...
        gpos = new GposTable(table, layout);
        return true;
    }
