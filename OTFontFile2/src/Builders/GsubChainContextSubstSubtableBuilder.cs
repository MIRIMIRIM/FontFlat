...
/// <summary>
/// Mutable builder for GSUB ChainContextSubst subtables (lookup type 6).
/// </summary>
/// <remarks>
/// Currently supports format 3 (coverage-based) only.
/// </remarks>
public sealed class GsubChainContextSubstSubtableBuilder
{
    ...
    public int SubstCount => _records.Count;
    ...
    public void ClearSubstLookupRecords()
    {
        if (_records.Count == 0)
            return;
    ...
    public void AddSubstLookupRecord(ushort sequenceIndex, ushort lookupListIndex)
    {
    ...
    private byte[] BuildFormat3Bytes()
    {
        ...
        if (_records.Count > ushort.MaxValue) throw new InvalidOperationException("SubstCount must fit in uint16.");

        int backCount = _backtrack.Count;
        int inputCount = _input.Count;
        int lookCount = _lookahead.Count;
        int substCount = _records.Count;
