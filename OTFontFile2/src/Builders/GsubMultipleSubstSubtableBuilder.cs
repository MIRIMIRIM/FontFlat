...
/// <summary>
/// Mutable builder for GSUB MultipleSubst subtables (lookup type 2), format 1.
/// </summary>
public sealed class GsubMultipleSubstSubtableBuilder
{
    ...
    public void AddOrReplace(ushort fromGlyphId, ReadOnlySpan<ushort> substitutes)
    {
        if (substitutes.Length == 0)
            throw new ArgumentException("MultipleSubst substitutes must be non-empty.", nameof(substitutes));
        ...
        _entries.Add(new Entry(fromGlyphId, substitutes.ToArray()));
        MarkDirty();
    }
    ...
    private byte[] BuildBytes()
    {
        ...
        if (uniqueCount > ushort.MaxValue)
            throw new InvalidOperationException("SequenceCount must fit in uint16.");
        ...
        Span<OTFontFile2.OffsetWriter.Label> sequenceLabels = uniqueCount <= 128
        ...
        {
            w.Align2();
            w.DefineLabelHere(sequenceLabels[i]);

            var subs = entries[i].Substitutes;
            if (subs.Length == 0)
                throw new InvalidOperationException("Sequence glyphCount must be >= 1.");
            if (subs.Length > ushort.MaxValue)
                throw new InvalidOperationException("Sequence glyphCount must fit in uint16.");

            w.WriteUInt16(checked((ushort)subs.Length));
            for (int s = 0; s < subs.Length; s++)
                w.WriteUInt16(subs[s]);
        }
    ...
    private readonly struct Entry
    {
        ...
        public ushort[] Substitutes { get; }

        public Entry(ushort fromGlyphId, ushort[] substitutes)
        {
