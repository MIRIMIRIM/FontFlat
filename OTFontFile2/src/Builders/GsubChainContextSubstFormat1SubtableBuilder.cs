...
/// <summary>
/// Mutable builder for GSUB ChainContextSubst subtables (lookup type 6), format 1 (rule-based).
/// </summary>
public sealed class GsubChainContextSubstFormat1SubtableBuilder
{
    ...
    private byte[] BuildBytes()
    {
        ...
        if (setCount > ushort.MaxValue)
            throw new InvalidOperationException("ChainSubRuleSetCount must fit in uint16.");
        ...
        for (int i = 0; i < setCount; i++)
        {
            ...
            if (rules.Count > ushort.MaxValue)
                throw new InvalidOperationException("ChainSubRuleCount must fit in uint16.");
