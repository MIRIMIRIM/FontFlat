...
/// <summary>
/// Mutable builder for GSUB ContextSubst subtables (lookup type 5), format 1 (rule-based).
/// </summary>
public sealed class GsubContextSubstFormat1SubtableBuilder
{
        ...
        ReadOnlySpan<SequenceLookupRecord> substLookupRecords)
    {
        for (int i = 0; i < _ruleSets.Count; i++)
        {
            ...
            _ruleSets[i].Rules.Add(new Rule(inputGlyphIds.ToArray(), substLookupRecords.ToArray()));
            MarkDirty();
            return;
        }

        var rules = new List<Rule>(capacity: 1) { new Rule(inputGlyphIds.ToArray(), substLookupRecords.ToArray()) };
    ...
    private byte[] BuildBytes()
    {
        ...
        if (setCount > ushort.MaxValue)
            throw new InvalidOperationException("SubRuleSetCount must fit in uint16.");
        ...
        for (int i = 0; i < setCount; i++)
        {
            ...
            if (rules.Count > ushort.MaxValue)
                throw new InvalidOperationException("SubRuleCount must fit in uint16.");
            ...
            for (int r = 0; r < ruleCount; r++)
            {
                ...
                if (glyphCount > ushort.MaxValue)
                    throw new InvalidOperationException("SubRule glyphCount must fit in uint16.");
                if (rule.Records.Length > ushort.MaxValue)
                    throw new InvalidOperationException("SubRule substCount must fit in uint16.");
