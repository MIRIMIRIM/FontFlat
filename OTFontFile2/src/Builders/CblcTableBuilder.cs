...
/// <summary>
/// Mutable builder for the OpenType <c>CBLC</c> table.
/// Layout-compatible with <c>EBLC</c>.
/// </summary>
[OtTableBuilder("CBLC", Mode = OtTableBuilderMode.Streaming)]
public sealed partial class CblcTableBuilder : ISfntTableSource
{
    ...
    private Fixed1616 _cbdtVersion = new(0x00020000u);
    ...
    public Fixed1616 Version
    {
        ...
        set
        {
            if (_isRaw)
            {
                ...
                if (bytes.Length < 4)
                    throw new InvalidOperationException("CBLC raw table data is too short.");
    ...
    {
        if (data.Length < 8)
            throw new ArgumentException("CBLC table must be at least 8 bytes.", nameof(data));
    ...
    public StrikeBuilder AddStrike(byte ppemX, byte ppemY, byte bitDepth = 32, sbyte flags = 0, uint colorRef = 0)
        => AddStrike(hori: default, vert: default, ppemX, ppemY, bitDepth, flags, colorRef);

    public bool TryBuildDerivedCbdt(out CbdtTableBuilder builder)
    {
        builder = null!;
        ...
        builder = new CbdtTableBuilder
        {
            Version = _cbdtVersion
        };
        builder.SetPayload(_builtCbdtPayload!);
        return true;
    }

    public static bool TryFrom(CblcTable cblc, out CblcTableBuilder builder)
    {
        builder = null!;

        var b = new CblcTableBuilder
        {
            Version = cblc.Version,
            BitmapSizeTableCount = cblc.BitmapSizeTableCount
        };

        var span = cblc.Table.Span;
        b._body = span.Length == 8 ? ReadOnlyMemory<byte>.Empty : span.Slice(8).ToArray();
        builder = b;
        return true;
    }

    public static bool TryFrom(CblcTable cblc, CbdtTable cbdt, out CblcTableBuilder builder)
    {
        builder = null!;

        if (cblc.BitmapSizeTableCount == 0)
            return false;

        if (!TryReadAllBytes(cbdt.Table, out byte[] cbdtBytes))
            return false;

        var b = new CblcTableBuilder
        {
            _version = cblc.Version,
            _cbdtVersion = cbdt.Version,
            _isStructured = true
        };

        uint sizeCountU = cblc.BitmapSizeTableCount;
        ...
        {
            if (!cblc.TryGetBitmapSizeTable(sizeIndex, out var size))
                return false;
            ...
            for (int subIndex = 0; subIndex < subCount; subIndex++)
            {
                ...
                for (uint glyphIdU = first; glyphIdU <= last; glyphIdU++)
                {
                    ...
                    if (!cbdt.TryGetGlyphSpan(ebdtOffset, length, out _))
                        return false;

                    st.SetGlyphData(glyphId, new ReadOnlyMemory<byte>(cbdtBytes, ebdtOffset, length));
                }
            }
    ...
    {
        if (_isRaw)
            throw new InvalidOperationException("CBLC builder is in raw mode; call EnableStructured() or use a new builder instance.");
    }

    private void EnsureStructured()
    {
        if (!_isStructured)
            throw new InvalidOperationException("CBLC builder is not in structured mode; call EnableStructured() first.");
    }

    private void EnsureNotStructured()
    {
        if (_isStructured)
            throw new InvalidOperationException("CBLC builder is in structured mode; use Strikes APIs instead of BodyBytes/SetBody.");
    }

    partial void OnMarkDirty()
    {
        _built = null;
        _builtCbdtPayload = null;
    }
    ...
    {
        if (!_isStructured)
            throw new InvalidOperationException("CBLC builder is not in structured mode.");

        if (_built is not null && _builtCbdtPayload is not null)
            return;

        BuildStructuredPair(out _built, out _builtCbdtPayload);
    }

    private void BuildStructuredPair(out byte[] cblcTable, out byte[] cbdtPayload)
    {
        int strikeCount = _strikes.Count;
        if (strikeCount == 0)
            throw new InvalidOperationException("CBLC structured builder must contain at least one strike.");

        int headerLen = checked(8 + (strikeCount * 48));

        int cblcLen = headerLen;
        int cbdtPayloadLen = 0;
        ...
        for (int i = 0; i < strikeCount; i++)
        {
            ...
            if (subCount == 0)
                throw new InvalidOperationException("CBLC strike must contain at least one index subtable.");
            ...
            for (int j = 0; j < subCount; j++)
            {
                ...
                cbdtPayloadLen = checked(cbdtPayloadLen + dataLen);
            }

            cblcLen = checked(cblcLen + strikeTablesLen);
        }

        cblcTable = new byte[cblcLen];
        cbdtPayload = new byte[cbdtPayloadLen];

        var cblc = cblcTable.AsSpan();
        BigEndian.WriteUInt32(cblc, 0, _version.RawValue);
        BigEndian.WriteUInt32(cblc, 4, checked((uint)strikeCount));

        int cblcCursor = headerLen;
        int cbdtCursor = 0;
        ...
        for (int strikeIndex = 0; strikeIndex < strikeCount; strikeIndex++)
        {
            ...
            BigEndian.WriteUInt32(cblc, sizeOffset + 0, checked((uint)cblcCursor)); // indexSubTableArrayOffset
            ...
            BigEndian.WriteUInt32(cblc, sizeOffset + 4, checked((uint)strikeTablesLen)); // indexTablesSize
            BigEndian.WriteUInt32(cblc, sizeOffset + 8, checked((uint)subCount)); // numberOfIndexSubTables
            BigEndian.WriteUInt32(cblc, sizeOffset + 12, strike.ColorRef);

            strike.Hori.WriteTo(cblc, sizeOffset + 16);
            strike.Vert.WriteTo(cblc, sizeOffset + 28);

            strike.ComputeGlyphRange(out ushort startGlyph, out ushort endGlyph);
            BigEndian.WriteUInt16(cblc, sizeOffset + 40, startGlyph);
            BigEndian.WriteUInt16(cblc, sizeOffset + 42, endGlyph);

            cblc[sizeOffset + 44] = strike.PpemX;
            cblc[sizeOffset + 45] = strike.PpemY;
            cblc[sizeOffset + 46] = strike.BitDepth;
            cblc[sizeOffset + 47] = unchecked((byte)strike.Flags);

            // IndexSubTableArray + IndexSubTables
            int arrayOffset = cblcCursor;
            ...
            for (int subIndex = 0; subIndex < subCount; subIndex++)
            {
                ...
                BigEndian.WriteUInt16(cblc, arrayEntryOffset + 0, st.FirstGlyphIndex);
                BigEndian.WriteUInt16(cblc, arrayEntryOffset + 2, st.LastGlyphIndex);

                uint additional = checked((uint)(subTableCursor - arrayOffset));
                BigEndian.WriteUInt32(cblc, arrayEntryOffset + 4, additional);

                int glyphCount = checked(st.LastGlyphIndex - st.FirstGlyphIndex + 1);
                int subTableLen = checked(8 + ((glyphCount + 1) * 4));

                // IndexSubTable header: indexFormat=1, imageFormat, imageDataOffset (into CBDT table).
                BigEndian.WriteUInt16(cblc, subTableCursor + 0, 1);
                BigEndian.WriteUInt16(cblc, subTableCursor + 2, st.ImageFormat);
                BigEndian.WriteUInt32(cblc, subTableCursor + 4, checked((uint)(4 + cbdtCursor)));

                // Offsets array + CBDT payload block.
                int offsetsOffset = subTableCursor + 8;
                int blockStart = cbdtCursor;
                int rel = 0;
                ...
                for (uint glyphIdU = st.FirstGlyphIndex; glyphIdU <= st.LastGlyphIndex; glyphIdU++)
                {
                    ...
                    BigEndian.WriteUInt32(cblc, offsetsOffset + (glyphIndex * 4), checked((uint)rel));
                    ...
                    if (st.TryGetGlyphData(glyphId, out var data))
                    {
                        ...
                        if (n != 0)
                        {
                            data.Span.CopyTo(cbdtPayload.AsSpan(cbdtCursor, n));
                            cbdtCursor = checked(cbdtCursor + n);
                            rel = checked(cbdtCursor - blockStart);
                        }
                    }
                }

                BigEndian.WriteUInt32(cblc, offsetsOffset + (glyphCount * 4), checked((uint)rel));

                subTableCursor = checked(subTableCursor + subTableLen);
            }

            cblcCursor = subTableCursor;
        }

        if (cblcCursor != cblcLen)
            throw new InvalidOperationException("CBLC builder internal length mismatch.");
        if (cbdtCursor != cbdtPayloadLen)
            throw new InvalidOperationException("CBDT builder internal length mismatch.");
    }
    ...
    {
        private readonly CblcTableBuilder _owner;
        ...
        internal StrikeBuilder(CblcTableBuilder owner, in LineMetrics hori, in LineMetrics vert, byte ppemX, byte ppemY, byte bitDepth, sbyte flags, uint colorRef)
        {
            _owner = owner;
            _hori = hori;
            _vert = vert;
        ...
        public LineMetrics HorizontalLineMetrics
        {
            get => _hori;
            set
            {
        ...
        public LineMetrics VerticalLineMetrics
        {
            get => _vert;
            set
            {
    ...
    {
        private readonly CblcTableBuilder _owner;
        private ushort _firstGlyphIndex;
        ...
        internal IndexSubTableBuilder(CblcTableBuilder owner, ushort firstGlyphIndex, ushort lastGlyphIndex, ushort imageFormat)
        {
            _owner = owner;
        ...
        public int GlyphRecordCount => _glyphData.Count;
        ...
        public bool RemoveGlyphData(ushort glyphId)
        {
            bool removed = _glyphData.Remove(glyphId);
            if (removed)
                _owner.MarkDirty();
            return removed;
        }
