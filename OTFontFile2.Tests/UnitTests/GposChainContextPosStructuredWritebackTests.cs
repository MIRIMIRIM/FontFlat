...
[TestClass]
public sealed class GposChainContextPosStructuredWritebackTests
{
    [TestMethod]
    public void FontModel_CanWriteStructuredGpos_WithChainContextPosFormat3()
    {
        ...
        single.AddOrReplace(glyphId: 10, value: new GposValueRecordBuilder { XAdvance = 321 });

        var lookup0 = gposBuilder.Layout.Lookups.AddLookup(lookupType: 1, lookupFlag: 0);
        lookup0.AddSubtable(single.ToMemory());

        var inputCov = new CoverageTableBuilder();
        inputCov.AddGlyph(10);

        var chain = new GposChainContextPosSubtableBuilder();
        chain.AddInputCoverage(inputCov);
        chain.AddPosLookupRecord(sequenceIndex: 0, lookupListIndex: 0);

        var lookup1 = gposBuilder.Layout.Lookups.AddLookup(lookupType: 8, lookupFlag: 0);
        lookup1.AddSubtable(chain.ToMemory());
        ...
        Assert.AreEqual((ushort)0, fmt3.BacktrackGlyphCount);

        Assert.IsTrue(fmt3.TryGetInputGlyphCount(out ushort inputCount));
        Assert.AreEqual((ushort)1, inputCount);

        Assert.IsTrue(fmt3.TryGetInputCoverage(0, out var cov));
        ...
        Assert.IsTrue(fmt3.TryGetLookaheadGlyphCount(out ushort lookCount));
        Assert.AreEqual((ushort)0, lookCount);

        Assert.IsTrue(fmt3.TryGetPosCount(out ushort posCount));
