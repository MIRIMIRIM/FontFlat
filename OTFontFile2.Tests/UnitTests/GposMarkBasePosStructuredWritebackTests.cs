...
[TestClass]
public sealed class GposMarkBasePosStructuredWritebackTests
{
    [TestMethod]
    public void FontModel_CanWriteStructuredGpos_WithMarkBasePosLookup()
    {
        ...
        var baseAnchor = AnchorTableBuilder.Format1(x: 10, y: 20);

        var markBase = new GposMarkBasePosSubtableBuilder();
        markBase.AddOrReplaceMark(markGlyphId: 10, @class: 0, markAnchor: markAnchor);
        markBase.AddOrReplaceBaseAnchor(baseGlyphId: 20, @class: 0, baseAnchor: baseAnchor);

        var lookup = gposBuilder.Layout.Lookups.AddLookup(lookupType: 4, lookupFlag: 0);
        lookup.AddSubtable(markBase.ToMemory());
        ...
        Assert.IsTrue(GposMarkBasePosSubtable.TryCreate(gpos.Table, subtableOffset, out var subtable));
        Assert.AreEqual((ushort)1, subtable.PosFormat);
        Assert.AreEqual((ushort)1, subtable.ClassCount);

        Assert.IsTrue(subtable.TryGetAnchorsForGlyphs(markGlyphId: 10, baseGlyphId: 20, out bool positioned, out var outMarkAnchor, out var outBaseAnchor));
        ...
        Assert.AreEqual((short)200, outMarkAnchor.YCoordinate);
        ...
        Assert.AreEqual((ushort)1, outBaseAnchor.AnchorFormat);
        Assert.AreEqual((short)10, outBaseAnchor.XCoordinate);
        Assert.AreEqual((short)20, outBaseAnchor.YCoordinate);
    }
}
